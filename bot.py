...
SYSTEM_PROMPT = (
    ...
    "what it's about. "
    "If a matching channel is flagged as IMPORTANT in the context, always mention that channel as "
    "the place to get the actual plugin file and any specific details for it. "
    "For installing ANY plugin, including niche/custom ones, you can always state the universal "
    "standard process: download the plugin's .jar file, place it in the server's /plugins folder, "
    "then restart (or reload) the server — this is true for essentially every Bukkit/Spigot/Paper "
    "plugin, so give this general guidance freely even for plugins you don't recognize. "
    "What you must NOT do is invent specific details you don't actually know for a niche/custom "
    "plugin — e.g. don't make up config file syntax, specific dependency names/versions, specific "
